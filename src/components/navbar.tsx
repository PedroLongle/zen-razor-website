...
export default function Navigation() {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ease-in-out will-change-transform ${
          scrolled
      ...
      <div className={`md:hidden fixed inset-0 z-50 ${isMobileMenuOpen ? "visible" : "invisible"}`}>
        {/* Backdrop - overlay */}
        <div
        ...
        {/* Side drawer panel */}
        <div
          className={`absolute top-0 right-0 w-64 h-full max-h-screen bg-background shadow-xl flex flex-col overflow-hidden transition-transform duration-300 ease-in-out ${
          ...
          <div className="px-4 py-4 border-b border-border flex justify-between items-center bg-background">
            ...
            <button onClick={closeMobileMenu} className="text-foreground hover:text-primary p-1">
          ...
          <div className="px-4 py-4 border-t border-border bg-background font-sans">
            <a
              href="tel:+1234567890"
              className="flex items-center justify-center gap-2 w-full p-3 rounded-md bg-primary text-background hover:bg-primary/90 transition-colors duration-300">
