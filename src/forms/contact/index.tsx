...
export default function ContactForm() {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-accent/10 p-6 rounded-lg border border-border">
      {submitSuccess && <div className="mb-6 p-4 bg-green-50 border border-green-200 text-green-700 rounded">{t("successMessage")}</div>}

      {submitError && <div className="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded">{submitError}</div>}

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormInput label={t("fullName")} placeholder={t("yourName")} register={register("name")} error={errors.name} required />
        ...
        <PhoneInput label={t("phone")} register={register("phone")} error={errors.phone} defaultCountry="PT" />
