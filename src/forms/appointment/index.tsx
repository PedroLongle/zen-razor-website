...
export default function AppointmentForm({
  ...
  const {
    ...
    setValue,
    formState: { errors }
  } = useForm<AppointmentFormData>({ resolver: yupResolver(appointmentSchema), mode: "onBlur" })
  ...
  useEffect(() => {
    ...
    for (let i = prevMonthDays - 1; i >= 0; i--) {
      ...
      calendarDaysArray.push({ date: day, isCurrentMonth: false, monthIndex: prevMonthIndex, year: prevMonthYear, isPast })
    }
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      calendarDaysArray.push({ date: i, isCurrentMonth: true, monthIndex: month, year: year, isPast })
    }
    ...
    for (let i = 1; i <= remainingDays; i++) {
      ...
      calendarDaysArray.push({ date: i, isCurrentMonth: false, monthIndex: nextMonthIndex, year: nextMonthYear, isPast })
    }
  ...
  const isDateSelected = (day: CalendarDay) => {
    ...
    return selectedDate.day === day.date && selectedDate.month === day.monthIndex && selectedDate.year === day.year
  }
  ...
  const handleDateSelect = (day: CalendarDay) => {
    ...
    setSelectedDate({ day: day.date, month: day.monthIndex, year: day.year, formattedString })
    ...
    <div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="relative">
              ...
              {errors.serviceId && <p className="mt-1 text-red-500 text-sm">{errors.serviceId.message}</p>}
            </div>

            <div className="mt-6 p-6 bg-accent/10 rounded-lg border border-border font-sans flex-grow">
              <h3 className="font-medium mb-2">
                {t("serviceSelection")} {selectedService ? `- ${selectedService.name}` : ""}
              </h3>
              ...
              <div className="flex justify-between items-center mt-4">
                ...
                <span className="font-medium font-sans">
                  {selectedService?.duration || "-"} {commonT("min")}
                </span>
              </div>
              <div className="flex justify-between items-center mt-3">
                ...
                <span className="font-medium">
                  {selectedService?.price || "-"} {commonT("euro")}
                </span>
              </div>
              ...
              {selectedDate && (
                <div className="flex justify-between items-center mt-3 border-t border-border pt-3">
                  ...
                  <span className="font-medium">
                    {selectedDate.formattedString}
                    {selectedTime && ` at ${selectedTime}h`}
                  </span>
                </div>
              )}
            </div>
          ...
          <div className={`flex flex-col h-full ${!selectedService ? "opacity-50" : ""}`}>
            ...
            <p className="text-muted-foreground mb-4 text-sm font-sans">{selectedService ? t("selectBarber") : t("selectService")}</p>
            <input type="hidden" {...register("barberId")} />
            {errors.barberId && <p className="mb-2 text-red-500 text-sm">{errors.barberId.message}</p>}
              ...
              <div className="grid grid-cols-7 text-center">
                ...
                {calendarDays.map((day, index) => (
                  <div
                    ...
                    {isDateSelected(day) && <div className="absolute inset-x-0 bottom-0 h-1 bg-primary"></div>}
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className={`mb-8 ${!selectedService || !selectedBarber || !selectedDate || !selectedTime ? "opacity-50" : ""}`}>
          ...
          <p className="text-muted-foreground mb-4 text-sm font-sans">
            {selectedService && selectedBarber && selectedDate && selectedTime ? t("personalInfo") : t("selectService")}
          </p>
